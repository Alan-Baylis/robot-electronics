/*
 HerkulexT.cpp - Library for Dongbu Herkulex DRS-0101/DRS-0201 with Teensy 3.x 
...
// Macro for Serial port selection
#define HSerial1     1 		// Write in Serial 1 port Teensy - Pin 0(Rx) - 1 (Tx) 
#define HSerial2     2   	// Write in Serial 2 port Teensy - Pin 9(Rx) - 10 (Tx) 
#define HSerial3     3   	// Write in Serial 3 port Teensy - Pin 7(Rx) - 8 (Tx)
 
// Only allow instantiation only if this is a Teensy 3.x
#if defined(__MK20DX128__) || defined(__MK20DX256__) || defined(__MKL26Z64__) || \
    defined(__MK64FX512__) || defined(__MK66FX1M0__) // 3.0/3.1-3.2/LC/3.5/3.6

// HerkulexT begin with Teensy - Serial 1
void HerkulexTClass::beginSerial1(long baud)
{
	Serial1.begin(baud);
	port = HSerial1;
}

// HerkulexT begin with Teensy - Serial 2
void HerkulexTClass::beginSerial2(long baud)
{
	Serial2.begin(baud);
	port=HSerial2;
}

// HerkulexT begin with Teensy - Serial 3
void HerkulexTClass::beginSerial3(long baud)
{
	Serial3.begin(baud);
	port = HSerial3;
}
...
// initialize servos
void HerkulexTClass::initialize()
{
        conta=0;
		lengthString=0;
        delay(100);       
...
// stat
byte HerkulexTClass::stat(int servoID)
{
	{
...
// torque on - 
void HerkulexTClass::torqueON(int servoID)
{
...
void HerkulexTClass::torqueOFF(int servoID)
{
	pSize = 0x0A;               // 3.Packet size 7-58
	pID   = servoID;            // 4. Servo ID
	cmd   = HRAMWRITE;          // 5. CMD
...
void HerkulexTClass::ACK(int valueACK)
{
	pSize = 0x0A;               // 3.Packet size 7-58
	pID   = 0xFE;	            // 4. Servo ID
	cmd   = HRAMWRITE;          // 5. CMD
...
void HerkulexTClass::set_ID(int ID_Old, int ID_New)
{
	pSize = 0x0A;               // 3.Packet size 7-58
...
// clearError
void HerkulexTClass::clearError(int servoID)
{
...
void HerkulexTClass::moveAll(int servoID, int Goal, int iLed)
{
	  if (Goal > 1023 || Goal < 0)
		return;						 //0 <--> 1023 range
...
// move all servo at the same time to a position: servo list building
void HerkulexTClass::moveAllAngle(int servoID, float angle, int iLed)
{
...
void HerkulexTClass::moveSpeedAll(int servoID, int Goal, int iLed)
{
...
void HerkulexTClass::actionAll(int pTime)
{
...
// get Position
 int HerkulexTClass::getPosition(int servoID) {
	int Position  = 0;
...
float HerkulexTClass::getAngle(int servoID) {
	int pos = (int)getPosition(servoID);
	return (pos-512) * 0.325;
}

// rollback servo to default settings
void HerkulexTClass::rollback(int servoID) {
...
void HerkulexTClass::reboot(int servoID) {
...
void HerkulexTClass::setLed(int servoID, int valueLed)
{
	pSize   = 0x0A;               // 3.Packet size 7-58
	pID     = servoID;            // 4. Servo ID
	cmd     = HRAMWRITE;          // 5. CMD
...
int HerkulexTClass::getSpeed(int servoID) {
  int speedy  = 0;
...
void HerkulexTClass::moveSpeedOne(int servoID, int Goal, int pTime, int iLed)
{
...
void HerkulexTClass::moveOne(int servoID, int Goal, int pTime, int iLed)
{
...
void HerkulexTClass::moveOneAngle(int servoID, float angle, int pTime, int iLed) {
...
void HerkulexTClass::writeRegistryRAM(int servoID, int address, int writeByte)
{
  pSize = 0x0A;               	// 3.Packet size 7-58
  pID   = servoID;     			// 4. Servo ID - 253=all servos
  cmd   = HRAMWRITE;          	// 5. CMD
...
void HerkulexTClass::writeRegistryEEP(int servoID, int address, int writeByte)
{
  pSize = 0x0A;                  // 3.Packet size 7-58
  pID   = servoID;     	         // 4. Servo ID - 253=all servos
  cmd   = HEEPWRITE;             // 5. CMD
...
// checksum1
int HerkulexTClass::checksum1(byte* data, int lengthString)
{
  XOR = 0;	
  XOR = XOR ^ pSize;
  XOR = XOR ^ pID;
...
// checksum2
int HerkulexTClass::checksum2(int XOR)
{
  return (~XOR)&0xFE;
}

// add data to variable list servo for syncro execution
void HerkulexTClass::addData(int GoalLSB, int GoalMSB, int set, int servoID)
{
...
void HerkulexTClass::sendData(byte* buffer, int length)
{
...
void HerkulexTClass::readData(int size)
{
	int i = 0;
    int beginsave=0;
    int Time_Counter=0;
...
void HerkulexTClass::clearBuffer()
{
  switch (port)
	{
	//#if defined (__AVR_ATmega1280__) || defined (__AVR_ATmega128__) || defined (__AVR_ATmega2560__)
...
void HerkulexTClass::printHexByte(byte x)
{
  Serial.print("0x");
  if (x < 16) {
    Serial.print('0');
